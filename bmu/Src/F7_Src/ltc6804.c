...
HAL_StatusTypeDef batt_readBackCellVoltage(float *cell_voltage_array, voltage_operation_t voltage_operation)
{
	...
	for (int board = 0; board < NUM_BOARDS_TESTING; board++){
		for(int ltc_chip = 0; ltc_chip < NUM_LTC_CHIPS_PER_BOARD; ltc_chip++) {
			size_t local_cell_idx = 0;
			for (int block = 0; block < NUM_LTC_CHIPS_PER_BOARD_TESTING; block++) {
				...
				for (int cvreg = 0; cvreg < VOLTAGES_PER_BLOCK; cvreg++)
				{
					...
					{
						// 5 AMS vlaues * 2 LTC chips * (3 voltage blocks *  3 voltages - 2) = 70 voltages
						// -2 from skipping terminal 4 and 5

						// populate each LTC's board's (10 LTC chips) correponging index (10 * 7)
						for (int i = 0; i < 10; i++){
							cell_voltage_array[cellIdx + 7 * i] = ((float)temp) / VOLTAGE_REGISTER_COUNTS_PER_VOLT;
