...
#define ERROR_FLOOR_RAD_S_DEFAULT (20.0f)
#define ERROR_FLOOR_PERCENT_DEFAULT (0.1f)
#define TORQUE_DEMAND_FLOOR_DEFAULT (2.0f)
...
float error_floor_rad_s = ERROR_FLOOR_RAD_S_DEFAULT;
float torque_demand_floor = TORQUE_DEMAND_FLOOR_DEFAULT;
...
void tractionControlTask(void *pvParameters)
{
	...
	float torque_adjustment = torque_demand_floor;
	...
	while(1)
	{
		...
		if(tc_on)
		{
			...
			const bool tc_active_left = ((wheel_speed_abs_error_left > error_floor_rad_s) && (wheel_speed_percent_error_left > ERROR_FLOOR_PERCENT_DEFAULT));
			const bool tc_active_right = ((wheel_speed_abs_error_right > error_floor_rad_s) && (wheel_speed_percent_error_right > ERROR_FLOOR_PERCENT_DEFAULT));
			const float desired_torque_adjustment_left = (wheel_speed_abs_error_left - error_floor_rad_s) * tc_kP;
			const float desired_torque_adjustment_right = (wheel_speed_abs_error_right - error_floor_rad_s) * tc_kP;
			...
			if(torque_max < torque_demand_floor)
			{
				torque_max = torque_demand_floor;
			}
