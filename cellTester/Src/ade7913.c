 ...
 * @brief   Function to read Cell  ADC.
 * @details Functions to read the Cell  ADC. The Cell  ADC measures the Cell bus
 * current, Cell  bus voltage, and Cell  battery voltage. The Cell  ADC is
 *connected to the BMU over SPI.
 *
 ******************************************************************************
 */
...
HAL_StatusTypeDef adc_spi_tx(uint8_t *tdata, unsigned int len) {
    HAL_GPIO_WritePin(CELL_ADC_CS_GPIO_Port, CELL_ADC_CS_Pin, GPIO_PIN_RESET);
    HAL_StatusTypeDef status =
        HAL_SPI_Transmit(&CELL_ADC_SPI_HANDLE, tdata, len, SPI_TIMEOUT);
    HAL_GPIO_WritePin(CELL_ADC_CS_GPIO_Port, CELL_ADC_CS_Pin, GPIO_PIN_SET);
    return status;
}

HAL_StatusTypeDef adc_spi_tx_rx(uint8_t *tdata, uint8_t *rbuffer,
                                unsigned int len) {
    HAL_GPIO_WritePin(CELL_ADC_CS_GPIO_Port, CELL_ADC_CS_Pin, GPIO_PIN_RESET);
    HAL_StatusTypeDef status = HAL_SPI_TransmitReceive(
        &CELL_ADC_SPI_HANDLE, tdata, rbuffer, len, SPI_TIMEOUT);
    HAL_GPIO_WritePin(CELL_ADC_CS_GPIO_Port, CELL_ADC_CS_Pin, GPIO_PIN_SET);
    return status;
}
...
uint8_t adc_readbyte(uint8_t addr, uint8_t *dataOut) {
    ...
    if (status != HAL_OK) {
        ERROR_PRINT("Error reading Cell ADC\n");
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef adc_read(uint8_t addr, int32_t *dataOut) {
     ...
     * rbuffer[3]);*/
    // put data into an u32. Skipping rbuffer[0] bc it contains high impedance
    // GARBAGE
...
HAL_StatusTypeDef adc_write(uint8_t addr, uint8_t data) {
    ...
    if (status != HAL_OK) {
        ERROR_PRINT("Error writing to Cell ADC!\n");
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef adc_read_current(float *dataOut) {
    ...
    /*DEBUG_PRINT("%.12f, ", shuntVoltage);*/
#ifdef DISABLE_CELL_WARNINGS
    // This should be "fixed"/removed when a smaller shunt resistor is found.
    // With our current setup we probably surpass the "nominal maximum" 31.25mV
    // of the ADC However, with this print statement in, the entire system
    // crashes when we surpass 312.5A
...
HAL_StatusTypeDef cell_adc_init() {
    ...
    do {
        if (adc_readbyte(ADDR_STATUS0, &status0) != HAL_OK) {
            ERROR_PRINT("Error reading Cell ADC status register\n");
            return HAL_ERROR;
        }
        vTaskDelay(5);
    ...
    if (adc_write(ADDR_CFG, cfgWrite) != HAL_OK) {
        ERROR_PRINT("Failed to config Cell  ADC\n");
        return HAL_ERROR;
    }

    uint8_t cfgRead;
    if (adc_readbyte(ADDR_CFG, &cfgRead) != HAL_OK) {
        ERROR_PRINT("Error reading Cell  ADC config register\n");
        return HAL_ERROR;
    }

    if (cfgRead != cfgWrite) {
        ERROR_PRINT("Cell ADC: config wrote %d != read %d\n", cfgWrite,
                    cfgRead);
        return HAL_ERROR;
    }
