/**
  ...
  * current, Cell  bus voltage, and Cell  battery voltage. The Cell  ADC is connected to
  * the BMU over SPI.
  *
  ******************************************************************************
  */
...
HAL_StatusTypeDef adc_spi_tx(uint8_t * tdata, unsigned int len) {
    ...
    HAL_StatusTypeDef status = HAL_SPI_Transmit(&CELL_ADC_SPI_HANDLE, tdata, len, SPI_TIMEOUT);
...
HAL_StatusTypeDef adc_spi_tx_rx(uint8_t * tdata, uint8_t * rbuffer, unsigned int len) {
    HAL_GPIO_WritePin(CELL_ADC_CS_GPIO_Port, CELL_ADC_CS_Pin , GPIO_PIN_RESET);
    HAL_StatusTypeDef status = HAL_SPI_TransmitReceive(&CELL_ADC_SPI_HANDLE, tdata, rbuffer, len, SPI_TIMEOUT);
    HAL_GPIO_WritePin(CELL_ADC_CS_GPIO_Port, CELL_ADC_CS_Pin , GPIO_PIN_SET);
    return status;
}
