...
void can_rx_task (void * pvParameters){
    ...
    dcu_hil_queue = xQueueCreate(MAX_QUEUE_LENGTH, sizeof(twai_message_t));
    ...
    while(1) {
        ...
        #ifdef DCU_HIL_ID
            error = xQueueSend(dcu_hil_queue, &rx_msg, portMAX_DELAY);
            if(error != pdPASS)
            {
                printf("failed to send message to DCU HIL\n");
            }
        #endif
